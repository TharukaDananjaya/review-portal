    ...
    <div class="max-w-md mx-auto bg-slate-100 dark:bg-slate-700 rounded-lg overflow-hidden shadow-lg">
        <div class="p-6">
            ...
            <div class="text-center">
                <button class="rounded-full bg-blue-400 p-2 hover:bg-blue-400 mt-2" id="uploadImage"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <!-- User Information -->
            <div class="mt-6">
                <form method="POST" action="@if (auth()->user()->type == 1) {{ route('admin.profile.update') }} @else {{ route('user.profile.update') }} @endif" class="mt-8" enctype="multipart/form-data">
                    @csrf
                    ...
                    {{-- <div class="mb-4">
                        ...
                        <input type="text" id="company_name" name="company_name" value="{{ $data['company_name'] }}"
                            required autofocus
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-400 focus:border-blue-400 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-400 dark:focus:border-blue-400 @error('comapny_name') border-red-500 @enderror">
                    </div> --}}
